...
export type MSWOnResponseMock = jest.MockedFunction<(res: unknown, requestId: string) => void>;

// start mock server
beforeAll(function () {
  const onRequest = jest.fn<never, Parameters<(req: MockedRequest) => void>>();
  const onResponse = jest.fn<never, Parameters<(res: unknown, requestId: string) => void>>();
