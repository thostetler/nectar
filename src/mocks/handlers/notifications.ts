...
export const notificationsHandlers = [
  ...
  // add
  http.post<{ id: string }>(apiHandlerRoute(ApiTargets.MYADS_NOTIFICATIONS), async ({ request }) => {
    const {
      type,
      template = null,
      data = null,
      ...
      frequency,
      name = 'added example',
    } = (await request.json()) as IADSApiAddNotificationParams;
  ...
  // edit
  http.put<{ id: string }>(apiHandlerRoute(ApiTargets.MYADS_NOTIFICATIONS, '/:id'), async ({ params, request }) => {
    const id = params.id;
