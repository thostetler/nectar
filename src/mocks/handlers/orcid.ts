...
export const orcidHandlers = [
  ...
  http.get(apiHandlerRoute(ApiTargets.ORCID_EXCHANGE_TOKEN), () =>
    HttpResponse.json(orcidExchangeTokenResponse),
  ),
  ...
  http.post(apiHandlerRoute(ApiTargets.ORCID_PREFERENCES), async ({ request }) => HttpResponse.json(await request.json())),
];
