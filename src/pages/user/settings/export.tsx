...
const ExportSettingsPage: NextPage = () => {
  ...
  const { data } = useSearch(SAMPLE_REQUEST_PARAMS);
...
const SAMPLE_REQUEST_PARAMS = getSearchParams({ q: 'bibstem:ApJ author_count:[10 TO 20]', rows: 1 });
...
export const getServerSideProps: GetServerSideProps = composeNextGSSP(async (ctx: GetServerSidePropsContext) => {
  ...
  const params = SAMPLE_REQUEST_PARAMS;
