...
describe('parseAPIError', () => {
  ...
  test('works', async ({ server }: TestContext) => {
    server.use(
      http.get('*test', () => HttpResponse.json({ message: 'Message' }, { status: 400 }), { once: true }),
      http.get('*test', () => HttpResponse.json({ error: 'Error' }, { status: 400 }), { once: true }),
      http.get('*test', () => HttpResponse.json({}, { status: 500 })),
    );
