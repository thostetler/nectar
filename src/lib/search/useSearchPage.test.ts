...
// DefaultProviders requires options prop — pass empty object since all fields are optional.
const makeWrapper = (searchParams: string) => {
  const NuqsWrapper = withNuqsTestingAdapter({ searchParams });
  const Wrapper = ({ children }: { children: React.ReactNode }) =>
    React.createElement(NuqsWrapper, null, React.createElement(DefaultProviders, { options: {} }, children));
